...
import { BiArrowFromLeft, BiArrowToRight, BiLoader } from "react-icons/bi";
import { BsFillMenuButtonWideFill, BsGraphDown } from "react-icons/bs";
import {
  FaArrowRight,
  FaDownload,
  FaMicrophone,
...
import { GrOptimize, GrResume } from "react-icons/gr";
...
const features = [
  ...
  },
  {
    ...
    number: "95%",
    des: "Higher chance of getting hired",
  ...
  },
  {
    ...
    number: "24/7",
    des: "Your smart career companion",
    logo: <MdOutlineHeadsetMic />,
...
const functionalities = [
  ...
  },
  {
    ...
    description:
      "Get an ATS score and persomalized tips to improve your resume instantly.",
    icon: <BsGraphDown />,
    ...
    <div className="flex flex-col items-center justify-center gap-6 mx-auto lg:mt-30 p-4 md:my-20 my-44 lg:min-h-screen ">
      ...
      {/* <div className="flex flex-row items-center justify-center w-full gap-8">
          {features.map((value) => (
            <div
              key={value.name}
              className="flex flex-row items-center gap-4 p-4 border-r border-gray-200 w-84 h-32 "
            >
              <p
                className={`text-3xl ${value.logoColor} ${value.logoBgColor} rounded-4xl p-4`}
              >
                {value.logo}
              </p>
              <div className="flex flex-col">
                <p className="text-4xl text-black py-2">{value.number}</p>
                <h3 className="text-lg font-regular">{value.name}</h3>
                <p className="text-sm text-gray-500">{value.des}</p>
              </div>
            </div>
          ))}
        </div> */}
